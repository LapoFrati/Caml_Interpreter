...
let rec  sem ((e:exp), (r)) =
      ...
      | Append(e1, e2) -> (match (sem(e1,r), sem(e2,r)) with
            							...
            							| _ -> failwith("type error"))
      | IfThenElse(e1, e2, e3) -> (match( sem(e2,r),sem(e3,r) )with (*controlla che il tipo nei due rami coincida*)
                                    | (Int(x), Int(y)) -> if( sem(e1,r) = True) then Int(x) else Int(y)
                                    | (True, False) -> if( sem(e1,r) = True) then True else False
                                    | (True, True) -> True
                                    | (False, False) -> False
                                    | (False, True) -> if( sem(e1,r) = True) then False else True
                                    | (List(x), List(y)) -> if( sem(e1,r) = True) then List(x) else List(y)
                                    | (Fun(x), Fun(y)) -> if( sem(e1,r) = True) then Fun(x) else Fun(y)
                                    | (_,_) -> failwith("branch type mismatch"))
      ...
      | Apply(e1, e2) -> ( match(sem(e1,r))with (*controllo che e1 sia una Fun*)
                          | Fun(f1) -> let newenv = bind(r, "par", sem(e2,r)) (* Bindo la sem di e2 al paramentro formale par *)
