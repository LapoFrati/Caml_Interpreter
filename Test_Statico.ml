...
#use "Prog_Ocaml_Statico.ml";;

#use "assert.ml";;

print_endline ("Testing:");;

(* funz e funzenv formano la chiusura che sarà associata ad f1, sono scritte separatamente per leggibilità *)

let ffunzenv = bindlist(emptyenv(Int(0)),["x";"y";"t";"f";"l"],[Int(4);Int(5);True;False;List(Cons(3,Cons(2,Cons(1,Nil))))]);;
let ffunz = fun b -> Op("+",Den(b), Eval(Int(1)));;

let gfunzenv = bindlist(emptyenv(Int(0)),["x";"y";"t";"f";"l";"f1"],[Int(4);Int(5);True;False;List(Cons(3,Cons(2,Cons(1,Nil)))); Fun(ffunz, ffunzenv)]);;
let gfunz = fun b -> Op("+",Den(b), Den("x"));;

let r = bindlist(emptyenv(Int(0)),["x";"y";"t";"f";"l";"f1";"g1"],[Int(4);Int(5);True;False;List(Cons(3,Cons(2,Cons(1,Nil)))); Fun(ffunz, ffunzenv); Fun(gfunz, gfunzenv)]);;
...
let test () : bool = sem( LetIn ( "x", Eval(Int(10)) , Apply( Den("g1"), Eval(Int(38)))),r) = Int(42)
;; run_test "let x = 100 in g1( 38 ) -> 42 " test
